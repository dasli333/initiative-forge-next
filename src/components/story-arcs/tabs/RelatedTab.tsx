...
export function RelatedTab({ storyArcId, campaignId }: RelatedTabProps) {
  ...
  const { data: backlinks = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const mentions = await getMentionsOf('story_arc', storyArcId);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Other entities that reference this story arc
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground py-4">
            No mentions yet. Use @mentions in other entities to reference this story arc.
          </p>
        ) : (
