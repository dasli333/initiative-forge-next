...
/**
 * Upload story item image to Supabase Storage with WebP compression
 * @param campaignId - Campaign ID for folder structure
 * @param file - Image file to upload
 * @returns Public URL of uploaded image
 */
export async function uploadStoryItemImage(
  campaignId: string,
  file: File
): Promise<string> {
  const supabase = getSupabaseClient();
  ...
  if (error) {
    console.error('Failed to upload story item image:', error);
    throw error;
  }
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from('story-item-images').getPublicUrl(data?.path || fileName);

  return publicUrl;
}

/**
 * Delete story item image from Supabase Storage
 * @param imageUrl - Full public URL of the image
 */
export async function deleteStoryItemImage(imageUrl: string): Promise<void> {
  const supabase = getSupabaseClient();

  // Extract file path from URL
  const fileName = imageUrl.split('/story-item-images/')[1];

  if (!fileName) {
    console.error('Invalid story item image URL:', imageUrl);
    throw new Error('Invalid image URL');
  }
