...
export async function searchCampaignEntities(
  ...
  const allEntities: EntitySearchResult[] = [
    ...(locationsRes.data || []).map((loc) => ({
      ...
      excerpt: extractExcerpt(loc.description_json),
    })),
    ...(npcsRes.data || []).map((npc) => ({
      ...
      excerpt: extractExcerpt(npc.biography_json),
    })),
    ...(questsRes.data || []).map((quest) => ({
      ...
      imageUrl: null,
      excerpt: extractExcerpt(quest.description_json),
    })),
    ...(sessionsRes.data || []).map((session) => ({
      ...
      imageUrl: null,
      excerpt: extractExcerpt(session.plan_json),
    })),
    ...(storyArcsRes.data || []).map((arc) => ({
      ...
      imageUrl: null,
      excerpt: extractExcerpt(arc.description_json),
    })),
    ...(storyItemsRes.data || []).map((item) => ({
      ...
      excerpt: extractExcerpt(item.description_json),
    })),
    ...(factionsRes.data || []).map((faction) => ({
      ...
      imageUrl: null,
      excerpt: extractExcerpt(faction.description_json),
    })),
    ...(loreNotesRes.data || []).map((note) => ({
      ...
      imageUrl: null,
      excerpt: extractExcerpt(note.content_json),
    })),
  ];
